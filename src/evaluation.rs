...
pub fn evaluate(state: &GameState) -> i32 {
    ...
    for color in Color::all() {
        for piece in Piece::all() {
            let piece_bitboard = state.pieces[color as usize][piece as usize];

            let piece_count = piece_bitboard.count_ones() as i32;
            mg[color as usize] += piece_count * MIDDLEGAME_MATERIAL_VALUES[piece as usize];
            eg[color as usize] += piece_count * ENDGAME_MATERIAL_VALUES[piece as usize];
            phase += piece_count * PIECE_PHASE_WEIGHTS[piece as usize];
            ...
            for square in piece_bitboard.iter() {
                ...
                mg[color as usize] += MIDDLEGAME_PIECE_TABLES[piece as usize][pst_sq as usize];
                eg[color as usize] += ENDGAME_PIECE_TABLES[piece as usize][pst_sq as usize];
            }
        }
    }

    let mg_score = mg[side_to_move as usize] - mg[side_to_move.opposite() as usize];
    let eg_score = eg[side_to_move as usize] - eg[side_to_move.opposite() as usize];

    let mg_phase = phase.min(PHASE_LIMIT);
    let eg_phase = PHASE_LIMIT - mg_phase;
    let mut eval = (mg_score * mg_phase + eg_score * eg_phase) / PHASE_LIMIT;
    // tempo bonus
    eval += if side_to_move == Color::White {
        10
    } else {
        -10
    };
